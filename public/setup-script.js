// ══════════════════════════════════════════════════════════════════════════════
// ADMIRAL SETUP — JAVASCRIPT (länkad från setup.html)
// ══════════════════════════════════════════════════════════════════════════════
...
function restoreProgress(s) {
    if (s.urlInput) document.getElementById('urlInput').value = s.urlInput;
    if (s.manualAA) document.getElementById('hc-aa').value = s.manualAA;
    if (s.inpToken) document.getElementById('inp-token').value = s.inpToken;
    if (s.inpAA1) document.getElementById('inp-aa1').value = s.inpAA1;
    if (s.inpAA2) document.getElementById('inp-aa2').value = s.inpAA2;
    if (s.inpAA3) document.getElementById('inp-aa3').value = s.inpAA3;
...
function onUrlInput(el) {
    ...
    if (bmMatch) {
        chips.innerHTML += `<div class="e-chip"><span class="e-label">BM ID</span>${bmMatch[1]}</div>`;
    }
...
async function runPreCheck() {
    ...
    // Simulate checks
    const checks = ['exist', 'active', 'bm', 'payment', 'currency'];
    for (let i = 0; i < checks.length; i++) {
        const key = checks[i];
        await ms(500 + Math.random()*300);
        setRc(key, 'pass', '✓');
    }
...
function setRc(key, state, detail) {
    ...
    if (state === 'pass') {
        icon.textContent = '✓';
        rc.classList.add('pass');
    } else if (state === 'fail') {
        icon.textContent = '✕';
        rc.classList.add('fail');
    } else if (state === 'spin') {
        icon.textContent = '';
        icon.style.borderTopColor = 'var(--color-accent-primary)';
    }
